...
class NpyDataset(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        img = cv2.imread(join(self.embed_path, self.npy_files)) # (256, 256, 3)
        gt2D = cv2.imread(join(self.gt_path, self.npy_files))[:,:,0]
        ...
        return torch.tensor(input_image).float(), torch.tensor(gt2D[None, :,:]).long(), torch.tensor(bboxes).float()
...
for epoch in range(num_epochs):
    ...
    for step, (input_image, gt2D, boxes) in enumerate(tqdm(train_dataloader)):
